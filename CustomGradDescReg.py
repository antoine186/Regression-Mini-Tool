...
def simpreg_custom_graddesc(inputvar_dt, param_dt, obj_type, alpha, train_dt, label_dt):
    ...
    contain_err = np.zeros(len(train_dt))
    ...
    for i in range(len(train_dt)):
        ...
        for j in range(len(param_dt)):
            ...
            if (j == 1):
                ...
                param_dict[param_dt[j]] = param_dict[param_dt[j]] + (alpha * cur_err * inputvar_dict[inputvar_dt[j]])
